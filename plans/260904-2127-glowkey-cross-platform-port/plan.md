...
2,851 lines, holds 160 of the 194 tests, has zero platform crates, zero
...
| 2 | macOS builds and behaves identically; all 194 tests stay green | P1 |
...
| 7 | [Windows packaging and CI](./phase-07-windows-packaging.md) | Pending | 6 |
...
Phases 1-3 are pure refactor with **zero behaviour change** and are verifiable on
this machine. Phase 4 onward needs a Windows machine. Phases 8-10 are designed now
and built after Phase 6.
...
**Nothing in Phases 4-10 may be called done on a `cargo check`.** Phase 6 exists
because the owner has a Windows machine and will run it. Until then every Windows
behavioural claim in this plan is a hypothesis, and the phase files mark them as
such.
...
1. **Do the two platforms share one settings file?** Phase 2 forces the decision.
   The recommendation is per-platform hotkey storage inside one schema, but the
   owner may prefer separate files per platform entirely.
2. **Does Chrome on Windows exhibit the omnibox trailing-selection bug at all?**
   The macOS AX guard exists for a real, reproduced defect. Whether Windows needs
   an equivalent is unknown and is a Phase 6 measurement, not an assumption.
